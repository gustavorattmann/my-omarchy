...
check_api_key() {
    local test_request=$(curl -s "api.openweathermap.org/data/2.5/weather?q=Curitiba&appid=$API_KEY")
...
if [ -n "$WEATHER_DATA" ]; then
    ...
    # Format output
    TEXT="$ICON    $TEMP°C"
    TOOLTIP="Sensação térmica: $FEELS_LIKE°C\nTempo atmosférico: ${DETAILED_DESC^}"

    printf '{"text": "%s", "tooltip": "%s"}\n' "$TEXT" "$TOOLTIP"
else
    echo '{"text": "   --°C", "tooltip": "Clima indisponível"}'
fi